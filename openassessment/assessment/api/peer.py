...
def _validate_override_data(points_override, points_possible):
    """
    ...
    Returns:
        dict of:
            'success': (boolean)
            'msg': (string) error message if data check failed.
    """
    try:
        ...
        return {
            'success': False,
            'msg': 'An error was encountered creating the override score.',
        }
        
    try:
        float(points_override)
    except ValueError:
        return {
            'success': False,
            'msg': 'Please check that you have entered a valid score.',
        }

    if float(points_override) > float(points_possible):
        return {
            'success': False,
            'msg': 'You have entered a score greater than the maximum possible.',
        }
    elif float(points_override) < 0:
        return {
            'success': False,
            'msg': 'You have entered a score less than zero.',
        }
    else:
        return {
            'success': True,
