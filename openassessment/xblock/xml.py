...
def parse_assessments_xml(assessments_root):
    ...
    for assessment in assessments_root.findall('assessment'):
        ...
        if 'track_changes' in assessment.attrib:
            trackchanges_url = assessment.get('track_changes', '')
            assessment_dict['track_changes'] = trackchanges_url

            if trackchanges_url and not URLValidator.regex.match(trackchanges_url):
